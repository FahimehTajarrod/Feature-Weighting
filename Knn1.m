...
mdl = ClassificationKNN.fit(TrainingSet,GroupTrain,...
    'Distance',@(x,Z)chiSqrDist(x,Z,Weight),'NumNeighbors',12);
mdl1 = ClassificationKNN.fit(TrainingSet,GroupTrain,...
    'Distance',@(x,Z)chiSqrDist(x,Z,NoWeights),'NumNeighbors',12);
...
CorrectRate_Weighting_Chi=((nnz(result-truelabels))*100)/length(result)/100;
