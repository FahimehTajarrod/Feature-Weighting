...
TrainingSet=dataset(1:100,classcul);
TestSet=dataset(101:140,1:classcul-1);
truelabels=dataset(101:140,classcul);
...
for j=1:size(TestSet,1)
    for k=1:numClasses
        classout=svmclassify(models(k),TestSet(j,:));
        if(classout) 
            break;
        end
    end
...
CorrectRate=((nnz(result-truelabels))*100)/length(result)/100;